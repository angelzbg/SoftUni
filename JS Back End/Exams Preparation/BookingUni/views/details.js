            ...
            <div class="hotel-right">
                ...
                ${
                  user._id.equals(data.owner)
                    ? `<a href="/edit/${data._id}" class="edit" onclick="showLoading(event)">Edit</a>
                        <a href="/delete/${data._id}" class="remove" onclick="showLoading(event)">Delete</a>`
                    : data.booked.findIndex((hId) => hId.equals(user._id)) !== -1
                    ? `<p><span class="green">You already have booked a room</span> </p>`
                    : `<a href="/book/${data._id}" class="book" onclick="showLoading(event)">Book</a>`
                }
            </div>
        </div>
    </section>`;
